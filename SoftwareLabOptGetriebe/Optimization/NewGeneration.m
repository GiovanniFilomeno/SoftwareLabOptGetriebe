function [r,y,theta,rotations,EC2_placement, Fitness, Best_fitness, Mean_fitness] = NewGeneration(pop,iter,r,y,theta,rotations,EC2_placement, Fitness)
% Function that takes the current set of individuals along with their
...
% two individuals are chosen(**SELECTION**), their set of variables combined(**CROSSOVER**)
% and the resulting offspring is altered a little(**MUTATION**) in order to
...
%                 defined value.
% :param int iter: Current iteration of the algorithm. 
% :param array r: Array with current generation values that define the perpendicular distance
%                 between the center of the electric motor and the respective ECs
% :param array y: Array with current generation values that define the y-axis
%                 location of the ECs.
% :param array theta: Array with current generation values between 0 and 2*pi that
%                     define the position of the ECs around the circular Electric Motor.
% :param array Rotations: Array with current generation values that define the rotations 
%                         of the gearbox along its drive axis, as
%                         well as the rotation of the ECs along its own center.
% :param array EC2_placement: Array with current generation boolean values. Whether the value 
%                             is 1 or 0 defines if the Capacitor will be
%                             positioned to the left or right of the IGBT.
% :param array Fitness: Fitness of the current Generation. Used here to
%                       compute mean, worst and best fitness so the selection can be done.
%
% :return: 
%   *[r, y, theta, rotations, EC2_placement, Fitness, Best_fitness, Mean_fitness, Worst_fitness]*
%       - **r** : Updated array with new generation values that define the perpendicular distance
%         between the center of the electric motor and the respective ECs.
%       - **y** : Updated array with new generation values that define the y-axis
%         location of the ECs. Mutation can make some go beyond initial limits. 
%       - **theta** : Updated array with new generation values between 0 and 2*pi that
%         define the position of the ECs around the circular Electric Motor.
%       - **rotations** :Updated array with new generation values
%         that define the rotations of the gearbox along its drive axis, as
%         well as the rotation of the ECs along their own center.
%       - **EC2_placement** : Updated array with new generation boolean values.
%         Whether the value is 1 or 0 defines if the Capacitor will be
%         positioned to the left or right of the IGBT.
%       - **Fitness** : Updated fitness of the new Generation. Because best half of previous generation is preserved and the other half
%         is new, preserved individuals keep their fitness. This helps in
%         that for next :func:`EvaluateFitness` calls, only half of new
%         generation is evaluated so computing time is reduced.
%       - **Best_fitness** : Lowest value from Fitness array of the
%         previous generation. Used to measure the performance of the
%         algorithm
%       - **Mean_fitness** : Average value from Fitness array of the
%         previous generation. Used to measure the performance of the
%         algorithm and for further :func:`EvaluateFitness` to determine if
%         an individual can generate a spline.
%
%
%
% :rtype: [ double array, double array, double array, double array, double array, double array, double, double, double] 
%
% **Example in Code**
%
% .. code-block::
