function [length,max_curvature, curvature, collisionwith1, collisionwithgb] = spline_connection_gb (pt1, pt2, pt3, pt4,pt5, EC1_center, EM_start, EM_end, fv, EC1)
% igbt to motor
% 
%             im_spline = cscvn(im');
%             fnplt(im_spline,'y',2)
...
vector = pt4-pt3;
pt3a = pt3 + 0.25 .* vector ;
pt3b = pt3 + 0.5 .* vector ;
pt3c = pt3 + 0.75 .* vector ;

xyz = [pt1; pt2; pt3; pt3a; pt3b; pt3c; pt4; pt5];
...
iteration = 0;
max_curvature = 0.5;
while (collisionwith1>0 || collisionwithgb>0.0072 || max_curvature>0.15)
%xyz = [pt1; pt2; pt3; pt4];
...
collisiongb = [];
    ...
    for i=1:Spline.pieces
            ...
            for t=t1:increment:t2
                ...
                collisiongb(end+1) = inpolyhedron(fv, [x_t y_t z_t]);
            end
...
collisionwithgb = sum(collisiongb)/size(collisiongb,2)

    if (collisionwith1==0 && collisionwithgb==0.0071 && max_curvature<=0.15)
        break;
    end
