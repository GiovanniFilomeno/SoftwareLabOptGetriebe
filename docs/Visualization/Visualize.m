function Visualize(i,n, iter,bauraum, gearbox,Anchor, EC1,EC1_width, EC1_center, EC1_SurfCenter, EC2, EC2_center, EC2_SurfCenter, EC3, EC3_center, EC3_SurfCenter, EC4, EC4_center, EC4_SurfCenter, EM_start,EM_vector, Fitness,r,y,theta,Rotations,EC2_placement)
%Function that prints a MATLAB figure showing the best solution found by the optimization process for a defined generation of the genetic algorithm.
%
% :param int i: Counter that indicates which element of the population will be shown.
% :param int n: User defined value that tells how many solutions per generation will be shown.
% :param int iter: Indicates the number of generation from which the solution will be shown.
...
% :param array Anchor: Coordinates of a point at the center of the gearbox shaft, which will be used for rotations.
% :param struct EC1: struct that represents the IGBT geometry.
% :param array EC1_width: Width of the IGBT.
% :param array EC1_center: Vector of coordinates of the body center of the IGBT, used for rotations of the component.
% :param array EC1_SurfCenter: Array of vectors with coordinates of surface centers of the IGBT, used to calculate distance between components.
% :param struct EC2: struct that represents the Capacitor geometry.
% :param array EC2_center: Vector of coordinates of the body center of the Capacitor, used for rotations of the component.
% :param array EC2_SurfCenter: Array of vectors with coordinates of surface centers of the CCapacitor, used to calculate distance between components.
% :param struct EC3: struct that represents the EMI-filter geometry.
% :param array EC3_center: Vector of coordinates of the body center of the EMI-filter, used for rotations of the component.
% :param array EC3_SurfCenter: Array of vectors with coordinates of surface centers of the EMI-filter, used to calculate distance between components.
% :param struct EC4: struct that represents the Control board geometry.
% :param array EC4_center: Vector of coordinates of the body center of the Control board, used for rotations of the component.
% :param array EC4_SurfCenter: Array of vectors with coordinates of surface centers of the Control board, used to calculate distance between components.
...
% :param array Fitness: Array containing the fitness of the individuals for the current generation iter.
% :param array r: Array containing the r variable of the individuals for the current generation iter.
% :param array y: Array containing the y variable of the individuals for the current generation iter.
% :param array theta: Array containing the theta variable of the individuals for the current generation iter.
% :param array Rotations: Array containing the rotation variable of the individuals for the current generation iter.
% :param array EC2_placement: Array containing the position of the Capacitor for the individuals for the current generation iter.   
%
% **Example in Code**
%
% .. code-block:: 
...
%
    ...
    [srt,I] = sort(Fitness);
    gearbox.vertices = Rotation(gearbox.vertices, Anchor, 2, Rotations(I(i,1),1));
    EM_start = Rotation(EM_start, Anchor, 2, Rotations(I(i,1),1));
    EM_end = EM_start + EM_vector;

    coords1(I(i,1),:) = [EM_start(1)+ r(I(i,1),1).*sin(theta(I(i,1),1)) y(I(i,1),1) EM_start(3) + r(I(i,1),1).*cos(theta(I(i,1),1))];
    coords2(I(i,1),:) = [EM_start(1)+ r(I(i,1),2).*sin(theta(I(i,1),2)) y(I(i,1),2) EM_start(3) + r(I(i,1),2).*cos(theta(I(i,1),2))];
    coords3(I(i,1),:) = [EM_start(1)+ r(I(i,1),3).*sin(theta(I(i,1),3)) y(I(i,1),3) EM_start(3) + r(I(i,1),3).*cos(theta(I(i,1),3))];
    coords4(I(i,1),:) = [EM_start(1)+ r(I(i,1),4).*sin(theta(I(i,1),4)) y(I(i,1),4) EM_start(3) + r(I(i,1),4).*cos(theta(I(i,1),4))];

    %IGBT

    EC1_mov = theta(I(i,1),1) + Rotations(I(i,1),2);
    EC1.vertices = EC1.vertices - EC1_center + coords1(I(i,1),:);
    EC1_SurfCenter(1,:) = EC1_SurfCenter(1,:) - EC1_center + coords1(I(i,1),:);
    EC1_SurfCenter(2,:) = EC1_SurfCenter(2,:) - EC1_center + coords1(I(i,1),:);
    EC1_SurfCenter(3,:) = EC1_SurfCenter(3,:) - EC1_center + coords1(I(i,1),:);
    EC1_SurfCenter(4,:) = EC1_SurfCenter(4,:) - EC1_center + coords1(I(i,1),:);
    EC1_center = coords1(I(i,1),:);
    ...
    EC2_mov = theta(I(i,1),2) + Rotations(I(i,1),3);
    EC2.vertices = EC2.vertices - EC2_center + coords2(I(i,1),:);
    EC2_SurfCenter(1,:) = EC2_SurfCenter(1,:) - EC2_center + coords2(I(i,1),:);
    EC2_SurfCenter(2,:) = EC2_SurfCenter(2,:) - EC2_center + coords2(I(i,1),:);
    EC2_SurfCenter(3,:) = EC2_SurfCenter(3,:) - EC2_center + coords2(I(i,1),:);
    EC2_SurfCenter(4,:) = EC2_SurfCenter(4,:) - EC2_center + coords2(I(i,1),:);
    EC2_center = coords2(I(i,1),:);
    ...
    EC3_mov = theta(I(i,1),3) + Rotations(I(i,1),4);
    EC3.vertices = EC3.vertices - EC3_center + coords3(I(i,1),:);
    EC3_SurfCenter(1,:) = EC3_SurfCenter(1,:) - EC3_center + coords3(I(i,1),:);
    EC3_SurfCenter(2,:) = EC3_SurfCenter(2,:) - EC3_center + coords3(I(i,1),:);
    EC3_SurfCenter(3,:) = EC3_SurfCenter(3,:) - EC3_center + coords3(I(i,1),:);
    EC3_SurfCenter(4,:) = EC3_SurfCenter(4,:) - EC3_center + coords3(I(i,1),:);
    EC3_center = coords3(I(i,1),:);
    ...
    %Control board
    EC4_mov = theta(I(i,1),4) + Rotations(I(i,1),5);
    EC4.vertices = EC4.vertices - EC4_center + coords4(I(i,1),:);
    EC4_SurfCenter(1,:) = EC4_SurfCenter(1,:) - EC4_center + coords4(I(i,1),:);
    EC4_SurfCenter(2,:) = EC4_SurfCenter(2,:) - EC4_center + coords4(I(i,1),:);
    EC4_SurfCenter(3,:) = EC4_SurfCenter(3,:) - EC4_center + coords4(I(i,1),:);
    EC4_SurfCenter(4,:) = EC4_SurfCenter(4,:) - EC4_center + coords4(I(i,1),:);
    EC4_center = coords4(I(i,1),:);
    ...
    %         hold on
    if EC2_placement(I(i,1),1) == 1   %Counterclockwise        
