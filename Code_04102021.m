...
%Read Gearbox
FV=stlread("Gearbox1(Rotated2).stl");
V = FV.Points;
V1 = 1000.*V;
...
%Definition of the axis of the electric motor
[EM_start, EM_vector, Anchor] = FindingAxis("Gearbox1(Rotated2).stl");
...
% fv.vertices = rotation(fv.vertices,Anchor, 2, -5*pi/35);
% EM_start = rotation(EM_start, Anchor, 2, -5*pi/35);
% EM_vector = rotation(EM_vector, zero, 2, -5*pi/35);
% EM_end = EM_start + EM_vector;
% EM_shaftx = [EM_start(1,1) EM_start(1,1) + EM_vector(1,1)];
% EM_shafty = [EM_start(1,2) EM_start(1,2) + EM_vector(1,2)];
% EM_shaftz = [EM_start(1,3) EM_start(1,3) + EM_vector(1,3)];
...
for i= 1:360
    if (out_gearbox(i) == max_IN)
        ...
        valid_angles_radians = valid_angles.*(pi/180);
    end
end
