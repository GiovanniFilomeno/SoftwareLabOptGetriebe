...
[EM_start, EM_vector, Anchor] = FindingAxis("Gearbox1.stl");
...
EM_vector = rotation(EM_vector, zero, 2, first_angle);
EM_end = EM_start + EM_vector;
...
for i=1:pop
    ...
    EM_vector = rotation(EM_vector, zero, 2, gb_mov(i,1));
    EM_end = EM_start + EM_vector;
    
    %IGBT
    EC1.vertices = EC1.vertices - EC1_center + coords1(i,:);
    EC1_center = coords1(i,:);
    EC1_center1 = coords1(i,:) - [0 EC1_width 0];
    EC1_center2 = coords1(i,:) + [0 EC1_width 0];
    EC1_center3 = coords1(i,:) - [0 0 EC1_height];
    
    %Capacitor
    EC2.vertices = EC2.vertices - EC2_center + coords2(i,:);
    EC2_center = coords2(i,:);
    EC2_center1 = coords2(i,:) - [0 EC2_width 0];
    EC2_center2 = coords2(i,:) + [0 EC2_width 0];
    
    %EMI-filter
    EC3.vertices = EC3.vertices - EC3_center + coords3(i,:);
    EC3_center = coords3(i,:);
    EC3_center1 = coords3(i,:) - [0 EC3_width 0];
    
    %Control board
    EC4.vertices = EC4.vertices - EC4_center + coords4(i,:);
    EC4_center = coords4(i,:);
    EC4_center1 = coords4(i,:) + [0 0 EC4_height];
...
EM_vector = rotation(EM_vector, zero, 2, -gb_rot(pop,1));
EM_end = EM_start + EM_vector;
...
for i=1:pop/2
 ...
 for g = 2:iter
      for k = 1: pop/2
        ...
        childgb(1,1) = alpha*parentsgb(1,1) + (1-alpha)*parentsgb(2,1);
        childgb(2,1) = beta*parentsgb(1,1) + (1-beta)*parentsgb(2,1);
...
%       Mutation
        for i = 1:2
             ...
             mut_prob = rand();
             if mut_prob > 0.5 
                 childgb(i,1) = childgb(i,1) + (pi/180);
             else
                 childgb(i,1) = childgb(i,1) - (pi/180);
             end
        end
        ...
        gb_rot(k,1) = childgb(1,1);
        gb_rot(k+1,1) = childgb(2,1);
      end
...
%Test new generation
      ...
      fv.vertices = rotation(fv.vertices, Anchor, 2, -gb_rot(length(gb_rot),1));
      EM_start = rotation(EM_start, Anchor, 2, -gb_rot(length(gb_rot),1));
      EM_vector = rotation(EM_vector, zero, 2, -gb_rot(length(gb_rot),1));
      EM_end = EM_start + EM_vector;
        ...
        EM_vector = rotation(EM_vector, zero, 2, gb_rot(I(1,1),1));
        EM_end = EM_start + EM_vector;
        
        EC1.vertices = EC1.vertices - EC1_center + coords1(I(1,1),:);
        EC1_center = coords1(I(1,1),:);
        EC1_center1 = coords1(I(1,1),:) - [0 EC1_width 0];
        EC1_center2 = coords1(I(1,1),:) + [0 EC1_width 0];
        EC1_center3 = coords1(I(1,1),:) - [0 0 EC1_height];

        EC2.vertices = EC2.vertices - EC2_center + coords2(I(1,1),:);
        EC2_center = coords2(I(1,1),:);
        EC2_center1 = coords2(I(1,1),:) - [0 EC2_width 0];
        EC2_center2 = coords2(I(1,1),:) + [0 EC2_width 0];

        EC3.vertices = EC3.vertices - EC3_center + coords3(I(1,1),:);
        EC3_center = coords3(I(1,1),:);
        EC3_center1 = coords3(I(1,1),:) - [0 EC3_width 0];

        EC4.vertices = EC4.vertices - EC4_center + coords4(I(1,1),:);
        EC4_center = coords4(I(1,1),:);
        EC4_center1 = coords4(I(1,1),:) + [0 0 EC4_height];
